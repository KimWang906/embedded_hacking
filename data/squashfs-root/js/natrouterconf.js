...
function submit_twinip_and_reboot(F, wid)
{
	var hw = 'w'+wid+'hw';   
	...
	F.wanid.value = wid;
	ApplyReboot(F, 'twinip');

}

function submit_dmz_off_reboot(F, wid, is_twinip)
{
	if(F.opmode.value == DMZTWINIP_MODE_TWINIP)
	{
		F.wanid.value = wid;
		ApplyReboot(F, 'twinip');
	}
	else
		submit_twinipdmz(wid,F.opmode.value, is_twinip);
}
...
{ 
        ...
        { 
			...
			HideIt('fake_ip'); 
                ...
                HideIt('w'+wid+'twinip_apply'); 
                ShowIt('w'+wid+'other_apply'); 
        } 
        else if (mode == 2) 
        { 
                ShowIt('w'+wid+'hwaddr'); 
			...
			ShowIt('fake_ip'); 
                ...
                ShowIt('w'+wid+'twinip_apply'); 
                HideIt('w'+wid+'other_apply'); 
        } 
        else    
        { 
                HideIt('w'+wid+'hwaddr'); 
...
{ 
        ...
        else if ((F.ftp_port.value > 65535) || (F.ftp_port.value == 80) || (F.ftp_port.value == 21)) 
...
function ApplyPPPoERelay()
{
	document.natrouterconf_misc_fm.act.value = 'pppoe_relay';
	document.natrouterconf_misc_fm.submit();
}
