...
function ApplyReboot(F, value)
{
	...
	else if(value == 'twinip' && !confirm(MSG_TWINIP_CONFIRM_WARNING)) return;
	else if(value == 'wan_for_lan' && !confirm(MSG_WAN_FOR_LAN_WARNING)) return;
	else if(value == 'pptpvpn' && !confirm(MSG_PPTPVPN_REBOOT)) return;
	else if(value == 'qos' && !confirm(MSG_QOS_REBOOT)) return;
	else if(value == 'igmp' && !confirm(MSG_IGMP_REBOOT)) return;
	else if(value == 'srvnet' && !confirm(MSG_RESTART_CONFIRM_REBOOT)) return;
	else if(value == 'usb_mode' && !confirm(MSG_USB_MODE_WARNING)) return;
...
function PrintRemainTime(id,time,url,msg)
{
        var obj;
        ...
        time--;
        if(time)
                setTimeout("PrintRemainTime('"+id+"',"+time+",'"+url+"','"+msg+"' );",1000);
        else
        {
		if(msg)
                	alert(msg);
...
function PrintDot(id,count,max)
{
        ...
        if(obj)
        {
                if(count) obj.innerHTML += '.';
                else obj.innerHTML = '.';
        }

        if(count == 0) count = max;
        setTimeout("PrintDot('"+id+"',"+count+","+max+" );",500);
}
